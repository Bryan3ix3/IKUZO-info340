...
export function ProfileScreen() {
    ...
    const [editBioIsDisplayed, setEditBioIsDisplayed] = useState(false);
    ...
    const handleClick = () => { //handle edit bio click
      setEditBioIsDisplayed(!editBioIsDisplayed);
    }
    ...
    function updateBio() {  //handle "check" button
      if(textValue !== '') {
        const bioRef = ref(db, "Profile/bio");
        setTextValue("Loading...");
        firebaseSet(bioRef, textValue)
          .then(() => setTextValue(""))
          .catch((error) => setTextValue(error))
          .then(() => setEditBioIsDisplayed(false));
      } else {
        setEditBioPlaceholder("Please enter bio here.")
      }
    }
    ...
    let interestFormStyle = {display : 'none'};
    let hobbyFormStyle = {display : 'none'};
    if(editBioIsDisplayed) {
      editBioStyle = {display: 'block'};
    }
    if(interestsFormIsDisplayed) {
      interestFormStyle = {display: 'block'};
    }
    if(hobbiesFormIsDisplayed) {
      hobbyFormStyle = {display: 'block'};
    }
        ...
        <section className="content">
          <section className="d-flex justify-content-center">
            <section className="card">
              ...
              <section style={editBioStyle}>
                <section className="input-group mb-3">
                    <input type="text" className="form-control" placeholder={editBioPlaceHolder} aria-label="Bio" aria-describedby="basic-addon1" value={textValue} onChange={handleInput}/>
                </section>
                  ...
                  <section className="accordion-body">
                    <section className="card text-center">
                      <ul className="list-group list-group-flush">
                        ...
                        <section className="form-popup" id="myForm" style={interestFormStyle}>
                  ...
                  <section className="accordion-body">
                    <section className="card text-center">
                      <ul className="list-group list-group-flush">
                        ...
                        <section className="form-popup" id="myForm" style={hobbyFormStyle}>
